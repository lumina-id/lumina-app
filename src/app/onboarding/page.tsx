...
export default function OnboardingPage() {
  ...
  // Step 2: Check if gaze is over button - for visual feedback only
  // Actual click is triggered by blink detection in useFaceMesh
  useEffect(() => {
    ...
    const padding = 40; // Padding for easier targeting
    const isOver =
  ...
  }, [currentStep, gazeX, gazeY, blinkDetected]);

  // Step 3: Check if gaze is over start button - for visual feedback only
  // Actual click is triggered by blink detection in useFaceMesh
  useEffect(() => {
    if (currentStep !== 3) {
    ...
    setIsStep3Hovering(isOver);
  }, [currentStep, gazeX, gazeY]);
          ...
          <div className="flex flex-col items-center gap-[24px] w-full max-w-[500px]">
            ...
            <div className="relative">
              <button
                ...
                className={`relative px-[32px] py-[16px] rounded-[12px] text-white text-[16px] font-medium tracking-[-0.64px] transition-all duration-200 cursor-pointer ${
                  isHovering
                    ? "shadow-[0_0_20px_rgba(11,31,183,0.5)] scale-105"
                    : "hover:scale-102 hover:shadow-[0_4px_15px_rgba(11,31,183,0.25)]"
                } ${
                  blinkDetected ? "ring-4 ring-green-400 ring-opacity-50" : ""
                }`}
                style={{
                  background:
                    "linear-gradient(180deg, #0B1FB7 0%, #081787 100%)",
                }}
              >
          ...
          <div className="flex flex-col items-center gap-[24px] w-full max-w-[400px]">
            ...
            <div className="relative">
              <button
                ...
                className={`px-[32px] py-[14px] rounded-[12px] text-white text-[16px] font-medium tracking-[-0.64px] transition-all duration-200 cursor-pointer ${
                  isStep3Hovering
                    ? "shadow-[0_0_20px_rgba(11,31,183,0.5)] scale-105"
                    : "hover:scale-102 hover:shadow-[0_4px_15px_rgba(11,31,183,0.25)]"
                }`}
